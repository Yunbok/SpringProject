...
public class CommunityBoardController {
	...
	private static final String dataPath = "D:\\JavaBigData8th\\mywork_spring\\SpringProject_v1.7\\WebContent\\resources\\data\\";
	/**
	...
	public String SearchList(@RequestParam("offset") int offset, @RequestParam("pagelink") int pagelink ,
		...
		model.addAttribute("offset", offset);
		model.addAttribute("pagelink", pagelink);
		model.addAttribute("PagingCount",boardService.showPaging(pagelink,"/community_board_serch_list.do",search,searchtext));
		return "Community_board/List";
	}
	...
	public String DoWriteBoard(@ModelAttribute("boardBeanObjToWrite") CommunityBoardContentsBean boardBeanObjToWrite,@RequestParam("kategorie") String kategorie, Model model) {
	...
	public String DoUpdateBoard(@ModelAttribute("boardData")CommunityBoardContentsBean boardBeanObjToUpdate, @RequestParam("offset")int offset,
			@RequestParam("pagelink")int pagelink, Model model) {
		...
		return "redirect:community_board_view_contents.do?no="+boardBeanObjToUpdate.getNo()+"&offset="+offset+"&pagelink="+pagelink;
	}
	...
	public String reply(@RequestParam("no")int no,@RequestParam("content")String content, RedirectAttributes redirctAttributes,
		...
		replyService.getInsertReply(no,Integer.parseInt(rno),content,writer);
	...
	public String rereply(@RequestParam("no")int no,@RequestParam("rno")int rno,@RequestParam("content")String content, RedirectAttributes redirctAttributes,
			HttpServletRequest request, @RequestParam("offset")int offset, @RequestParam("pagelink")int pagelink,Model model){
		...
		replyService.getInsertRereply(no,rno,Integer.parseInt(sno),content,writer);
